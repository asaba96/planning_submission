...
Odometry Target::getTargetAtTime(double time) {
    ...
    if (_isStatic) {
        ...
        if (_trajectory.start_time > rosTimeTarget) {
            ROS_ERROR( "TargetPlanner: Looking up target pose in past");
            throw std::runtime_error("TargetPlanner: bad timestamps");
        }

        double secondsRaw = (rosTimeTarget - _trajectory.start_time).toSec();
        double numStepsRaw = secondsRaw / _trajectory.dt;
        ...
        Point poseStart = _trajectory.motion_points[index].pose.position;
        Point poseEnd = _trajectory.motion_points[index2].pose.position;

        Vector3 velStart = _trajectory.motion_points[index].twist.linear;
        Vector3 velEnd = _trajectory.motion_points[index2].twist.linear;
        ...
        // assume maintains same velocity
