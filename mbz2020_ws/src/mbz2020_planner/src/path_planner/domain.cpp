...
Domain::Domain(std::shared_ptr<Map> map,
...
std::vector<std::shared_ptr<Successor> > Domain::getSuccessors(std::shared_ptr<Successor> currNode) {
    ...
    for (int x = -maxAccelIn; x <= maxAccelIn; x++) {
        for (int y = -maxAccelIn; y <= maxAccelIn; y++) {
            for (int z = -maxAccelIn; z <= maxAccelIn; z++) {
                ...
                if (isValid(node)) {
                    std::shared_ptr<Successor> nextSucc =
                        std::make_shared<Successor>();
                    nextSucc->closed = 0;
                    nextSucc->done = false;
...
double Domain::getHeuristic(GroundedState node) {
    for (double t = node.time; t < _limits->maxTime; t += _Hdt) {
        ...
        double tx = getHeuristicValue(node.x,
                                      node.xVel,
                                      targetOdom.pose.pose.position.x,
                                      targetOdom.twist.twist.linear.x);
        double ty = getHeuristicValue(node.y,
                                      node.yVel,
                                      targetOdom.pose.pose.position.y,
                                      targetOdom.twist.twist.linear.y);
        double tz = getHeuristicValue(node.z,
                                      node.zVel,
                                      targetOdom.pose.pose.position.z,
...
bool Domain::isGoal(GroundedState node) {
    ...
    double poseDiff = std::sqrt(
        std::pow(goalOdom.pose.pose.position.x - node.x, 2) +
    ...
    double velDiff = std::sqrt(
        std::pow(goalOdom.twist.twist.linear.x - node.xVel, 2) +
...
bool Domain::isValidWithPrints(GroundedState node) {
    // ROS_WARN_STREAM("IN BOUNDS: " << isInBounds(node)  << " IS FREE: " << _map->isFree(node.x, node.y, node.z, true));
    return isValid(node);
}
...
bool Domain::isInBounds(GroundedState node) {
    if (node.x > this->_limits->maxX ||
        node.y > this->_limits->maxY ||
        node.z > this->_limits->maxZ) {
        // node outside max map
        return false;
    }

    if (node.x < this->_limits->minX ||
        node.y < this->_limits->minY ||
        node.z < this->_limits->minZ) {
        // node outside min map
        return false;
    }

    if (std::max({std::abs(node.xVel),
                  std::abs(node.yVel),
...
double Domain::getHeuristicSearch(std::shared_ptr<State> currState, GroundedState groundedCurrState) {
...
double Domain::getHeuristicForHSearch(GroundedState state) {
    ...
    double h = (SQRT2_MINUS_ONE * std::min(std::abs(state.x),
                                           std::abs(state.y)) +
                std::max(std::abs(state.x),
                         std::abs(state.y)));
    return h;
}

std::vector<std::shared_ptr<Successor> > Domain::getSuccessorsForHSearch(std::shared_ptr<Successor> currNode) {
    std::vector<std::shared_ptr<Successor> > resultList;
    std::shared_ptr<State> currState = currNode->state;
