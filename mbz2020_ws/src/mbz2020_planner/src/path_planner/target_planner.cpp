...
TargetPlanner::TargetPlanner(std::shared_ptr<Domain> env, std::shared_ptr<State> start, double eps)
    : open_(),
    _env(env)
{
    numSearches = 0;
...
bool TargetPlanner::replan() {
    ...
    while (ros::ok() && !open_.empty()) {
        /*if (std::chrono::high_resolution_clock::now() > this->stopTime) {
            ROS_WARN("TargetPlanner: ran out of time with %d searches", numSearches);
            return false;
           }*/
        ...
        if (goalNode != nullptr && goalNode->g <= open_.top()->f) {
            ...
            ROS_INFO_STREAM("Targetlanner: done replanning");
            ROS_INFO_STREAM("TargetPLanner: num nodes expanded: " << numExpand);
            ROS_INFO_STREAM("TargetPlanner: re-planning time: "
                            << std::chrono::duration_cast<std::chrono::milliseconds>(
                                rePlanEnd - replanStartTime).count()
                            << "ms.");
            ROS_INFO_STREAM("TargetPlanner: G of GoalNode: " << goalNode->g <<
                            " open V: " << open_.top()->f);
    ...
    ROS_ERROR_STREAM("TargetPlanner: NO PATH FOUND!! NODES EXPANDED: " << numExpand);
    return false;
}
